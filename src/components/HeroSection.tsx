...
export default function HeroSection({
  ...
  // Mobile & Memory Optimized Preview URL generator
  const createOptimizedPreviewUrl = (file: File): string => {
    const isImage = file.type.startsWith("image/");
    const isVideo = file.type.startsWith("video/");
    if (!isImage && !isVideo) return "";

    // Optimization: Files larger than threshold (15MB for images, 25MB for videos)
    // skip ObjectURL creation on mobile to save memory and prevent lag/crashes
    const MAX_IMAGE_PREVIEW_SIZE = 15 * 1024 * 1024; // 15 MB
    const MAX_VIDEO_PREVIEW_SIZE = 25 * 1024 * 1024; // 25 MB

    if (isImage && file.size > MAX_IMAGE_PREVIEW_SIZE) return "";
        ...
        if (items[i].type.indexOf("image") !== -1 || items[i].type.indexOf("video") !== -1) {
          ...
          if (file) {
            ...
            newFiles.push({
              ...
              file,
              previewUrl: createOptimizedPreviewUrl(file),
            });
          }
        }
      }
  ...
  const processFiles = (files: File[], category?: "image" | "video" | "file") => {
    ...
    for (const f of validFiles) {
      ...
      incoming.push({
        ...
        file: f,
        previewUrl: createOptimizedPreviewUrl(f),
      });
    }
          ...
          if (blob) {
            ...
            setSelectedFiles((prev) => [
              ...prev,
              {
                id: "cam-" + Date.now(),
                file: capturedFile,
                previewUrl: createOptimizedPreviewUrl(capturedFile),
              },
            ]);
            stopCamera();
          }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12 animate-fade-in text-slate-800 dark:text-slate-100" id="hero-upload-area">
      ...
      <div className="text-center mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-2 tracking-tight leading-tight">
          {homepageTitle}
        </h1>
        <p className="text-xs sm:text-base md:text-lg text-slate-500 dark:text-slate-400 font-medium">
          {homepageSubtitle}
        </p>
      </div>
        ...
        <AnimatePresence mode="wait">
          {selectedFiles.length === 0 ? (
            ...
            >
              ...
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight relative z-10 leading-snug">
                      ...
                      <div className="aspect-square rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-950 relative flex items-center justify-center border border-slate-100 dark:border-slate-850">
                        {item.file.type.startsWith("video/") ? (
                          item.previewUrl ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900 text-white p-2 text-center relative">
                              <Video className="w-8 h-8 text-blue-400 mb-1" />
                              <span className="text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-300 border border-blue-500/30">
                                VİDEO ({(item.file.size / (1024 * 1024)).toFixed(0)}MB)
                              </span>
                            </div>
                          )
                          ...
                          item.previewUrl ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900 text-white p-2 text-center relative">
                              ...
                              <span className="text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                                GÖRSEL ({(item.file.size / (1024 * 1024)).toFixed(0)}MB)
                              </span>
                            </div>
                          )
                        ) : (
